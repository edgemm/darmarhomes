...
class wm_twitter extends WP_Widget {
	...
	*/
	function __construct() {
		$id = 'wm-twitter';
		$name = '<span>' . WM_THEME_NAME . ' ' . __( 'Twitter', WM_THEME_TEXTDOMAIN_ADMIN ) . '</span>';;
		$widget_ops = array(
			'classname'   => 'wm-twitter',
			'description' => __( 'Displays your recent tweets', WM_THEME_TEXTDOMAIN_ADMIN )
			);
		$control_ops = array();
	...
	*/
	function form( $instance ) {
		...
		?>
		<p class="wm-desc"><?php _e( 'Displays recent tweets from specific Twitter account. Also displays Twitter account details. Tweets are being cached to reduce the page load.', WM_THEME_TEXTDOMAIN_ADMIN ) ?></p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'username' ); ?>"><?php _e( 'Twitter username:', WM_THEME_TEXTDOMAIN_ADMIN ) ?></label><br />
			<input class="widefat" id="<?php echo $this->get_field_id( 'username' ); ?>" name="<?php echo $this->get_field_name( 'username' ); ?>" type="text" value="<?php echo esc_attr( $username ); ?>" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e( 'Number of tweets to display:', WM_THEME_TEXTDOMAIN_ADMIN ) ?></label><br />
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'userinfo' ); ?>"><?php _e( 'Disable Twitter user info', WM_THEME_TEXTDOMAIN_ADMIN ); ?></label>
		</p>
		<?php
	} // /form
	...
	*/
	private function wm_get_tweet( $count, $username, $no_userinfo, $exclude_replies = true ) {
		...
		$cache = get_transient( WM_THEME_NAME . '_tweets_id_' . esc_attr( $username ) );

		if ( $cache ) {

			$userDetails = get_transient( WM_THEME_NAME . '_tweets_id_' . esc_attr( $username ) );
			$tweets = get_option( WM_THEME_NAME . '_tweets_' . esc_attr( $username ) );

		} else {

			$response = wp_remote_get( 'http://api.twitter.com/1/statuses/user_timeline.xml?include_rts=true&screen_name=' . esc_attr( $username ) );
			...
			if ( ! is_wp_error( $response ) ) {
				...
				if ( empty( $xml->error ) ) {
					if ( isset( $xml->status[0] ) ) {
						...
						set_transient( WM_THEME_NAME . '_tweets_id_' . esc_attr( $username ), $userDetails, WM_TWITTER_CACHE_EXPIRATION );
						update_option( WM_THEME_NAME . '_tweets_' . esc_attr( $username ), $tweets );
					}
				}
			}
		...
		if ( $out ) {
			...
			$out = do_shortcode( '[box color="red" icon="warning"]' . __( 'No tweets.', WM_THEME_TEXTDOMAIN ) . '[/box]' );
		}
